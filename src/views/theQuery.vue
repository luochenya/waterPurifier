  ...
  <div class="contents">
    <div class="MemberSearch">
      <div class="search-box">
        ...
        <input v-model="searchValue" placeholder="請您輸入關鍵字" />
    ...
    <el-card v-if="active != 3 && active != 4 && active != 5">
      <el-row>
        ...
        <el-col :md="12" :lg="10">
          <div class="btn">
            <div :class="active == 0 ? 'spanActive' : ''" class="item item-1" @click="activeClick(0)">
              <img src="@/assets/imgs/ExchangeMall.png" alt="" />
            </div>
            <div :class="active == 1 ? 'spanActive' : ''" class="item item-2" @click="activeClick(1)">
              <img src="@/assets/imgs/latestEvents.png" alt="" />
            </div>
          </div>
          <span :class="active == 2 ? 'spanActive' : ''" @click="activeClick(2)">兌換記錄</span>
        </el-col>
      </el-row>
    </el-card>
    <div class="task">
      ...
      <el-row v-if="active == 0" :gutter="20">
        <el-col :md="12" :xs="24" v-for="(item, index) in ExchangeMallList" :key="index">
          <div class="ExchangeMall">
            ...
            <div class="ExchangeMall_content" @click="openProductDetails(item)">
              <h2>{{item.title}}</h2>
              <h3>IB幣：{{item.price}}</h3>
            </div>
            <div class="ExchangeMall_button" @click="CommodityExchange(item.price)">兌換</div>
          </div>
        </el-col>
      </el-row>
      <!-- 最新活動 -->
      <el-row v-if="active == 1" :gutter="20">
        <el-col :md="12" :xs="24" v-for="(item, index) in latestEventsList" :key="index">
          <div class="latestEvents">
            <div class="latestEvents_img" @click="openActivityDetails(item)">
              <img :src="item.imgs" alt="" />
            </div>
            <div class="latestEvents_content" @click="openActivityDetails(item)">
              <h2>{{item.title}}</h2>
              <h3>{{item.price}}</h3>
            </div>
            <div v-if="item.status == 1" class="latestEvents_button" @click="ActivityClaimClick(item.price)">領取</div>
            <div v-if="item.status == 0" class="latestEvents_buttons">已領取</div>
          </div>
        </el-col>
      </el-row>
      <!-- 兌換記錄 -->
      <el-row v-if="active == 2">
        <el-col :md="24" v-for="(item, index) in ExchangeRecordList" :key="index">
          <div class="ExchangeRecord" :class="!item.status ? 'ExchangeRecordActive' : ''" @click="ExchangeRecordCLick(item)">
            <div class="ExchangeRecord_img">
              <img :src="item.imgs" alt="" />
            </div>
            <div class="ExchangeRecord_content">
              <h2>{{item.title}}</h2>
              <h3 v-if="item.CouponNumber">優惠價編號：{{item.CouponNumber}}</h3>
            </div>
          ...
          <div class="productDetails">
            <div class="productDetails_header">
              ...
              <div class="productDetails_button">
                <div class="productDetails_button_left" @click="active = 0">我知道了</div>
                <div class="productDetails_button_right" @click="CommodityExchange(ExchangeMallDetails.price)">兌換</div>
              </div>
            </div>
            <div class="productDetails_bottom">
              <h2>{{ExchangeMallDetails.details}}</h2>
        ...
        <el-col :md="24">
          <div class="activityDetails">
            <div class="activityDetails_header">
              ...
              <div class="activityDetails_button" v-if="latestEventsDetails.status == 1">
                <div class="activityDetails_button_left" @click="active = 1">我知道了</div>
                <div class="activityDetails_button_right" @click="ActivityClaimClick(latestEventsDetails.price)">領取</div>
              </div>
              <div class="activityDetails_buttons" v-if="latestEventsDetails.status == 0">
                <div class="activityDetails_button_left" @click="active = 1">我知道了</div>
                <div>
                  <div v-if="latestEventsDetails.couponStatus == 0" class="activityDetails_buttons_false" @click="couponClick(latestEventsDetails.price)">使用優惠卷</div>
                  <div v-if="latestEventsDetails.couponStatus == 1" class="activityDetails_buttons_true">優惠卷已使用</div>
                  <p v-if="latestEventsDetails.couponStatus == 1">無法再次使用此折扣</p>
                </div>
              </div>
            </div>
          ...
          <div class="activityDetails">
            <div class="activityDetails_header">
              ...
              <div class="activityDetails_buttons">
                <div class="activityDetails_button_left" @click="active = 2">我知道了</div>
                <div v-if="ExchangeRecordDetails.status" class="activityDetails_buttons_false" @click="ExchangeRecordClick()">已拿到實體產品</div>
              </div>
            </div>
            <div class="activityDetails_bottom">
              <h2>{{ExchangeRecordDetails.details}}</h2>
              <p v-html="ExchangeRecordDetails.Precautions"></p>
              <h4 v-if="ExchangeRecordDetails.status">您已於{{ExchangeRecordDetails.time}}通知，有任何問題請洽</h4>
            </div>
          </div>
        </el-col>
      </el-row>
    ...
    <!-- 最新活動無數據 -->
    <el-card class="noDataStyle" v-if="active == 1 && latestEventsList.length == 0 && !SearchStatus">
      <img src="@/assets/imgs/noData.png" alt="" />
      <p>尚未開放·敬請期待</p>
    </el-card>
    <!-- 兌換商城無數據 -->
    <el-card class="noDataStyle" v-if="active == 0 && ExchangeMallList.length == 0 && !SearchStatus">
      <img src="@/assets/imgs/noData.png" alt="" />
      <p>尚無商品</p>
    </el-card>
    <!-- 兌換記錄無數據 -->
    <el-card class="noDataStyle" v-if="active == 0 && ExchangeRecordList.length == 0 && !SearchStatus">
      <img src="@/assets/imgs/noData.png" alt="" />
      <p>尚無兌換券</p>
    </el-card>
    <!-- 查詢無數據 -->
    ...
    <!-- 商品兌換彈窗 -->
    <PopUps :isDialogShow="CommodityExchangeStatus" modyType="1" :noShowClick="CommodityExchangeClose" :submitClick="CommodityExchangeSubmit"></PopUps>
    <!-- 商品兌換成功彈窗 -->
    <PopUps :isDialogShow="SuccessfulRedemptionStatus" modyType="2" :noShowClick="SuccessfulRedemptionClose"></PopUps>
    <!-- 商品兌換失敗彈窗 -->
    <PopUps :isDialogShow="ErrorfulRedemptionStatus" modyType="3" :noShowClick="ErrorfulRedemptionClose"></PopUps>
    
    <!-- 活動領取彈窗 輸入推薦碼 -->
    <PopUp :isDialogShow="ActivityClaimStatus" modyType="0" :noShowClick="ActivityClaimClose" :submitClick="ActivityClaimSubmit">
      <template v-slot:body>
        <h3>請輸入推薦碼</h3>
        <input type="text" v-model="ReferralCode" placeholder="請輸入您的推薦碼" />
        <em v-if="ReferralCodeStatus"><i></i>無效推薦碼</em>
      </template>
    </PopUp>
    <!-- 活動領取成功彈窗 -->
    <PopUp :isDialogShow="ActivityClaimSuccessStatus" modyType="1" :noShowClick="ActivityClaimSuccessClose"></PopUp>
    <!-- 使用優惠價彈窗 輸入店號 -->
    <PopUp :isDialogShow="couponStatus" modyType="0" :noShowClick="couponClose" :submitClick="couponSubmit">
      <template v-slot:body>
        <h3>請由店家輸入店號</h3>
        <input type="text" v-model="CouponNumber" placeholder="請輸入店號" />
        <em v-if="CouponNumberStatus"><i></i>無效店號</em>
      </template>
    </PopUp>
    
    <!-- 已確認收貨彈窗彈窗 -->
    <PopUpss :isDialogShow="ConfirmReceiptStatus" modyType="1" :noShowClick="ConfirmReceiptClose" :submitClick="ConfirmReceiptSubmit"></PopUpss>
...
export default {
  ...
  data () {
    return {
      ...
      ExchangeMallList: [
        {
          imgs: require("@/assets/imgs/product.png"),
          title: '大苑子·翡翠檸檬鞏固 濾心可用兌換券1',
          price: 400,
          details: '一年內不管要换幾次濾心樱花都幫你换I凡購買指定機種，還享獨家優惠',
          Precautions: ' 注意事項:<br />1.本公司保有最終更改決定權。<br />2.台北信羲.桃概第一航廈等門市不提供兌换服務。'
        },{
          imgs: require("@/assets/imgs/product.png"),
          title: '大苑子·翡翠檸檬鞏固 濾心可用兌換券可用兌換券2',
          price: 400,
          details: '一年內不管要换幾次濾心樱花都幫你换I凡購買指定機種，還享獨家優惠',
          Precautions: ' 注意事項:<br />1.本公司保有最終更改決定權。<br />2.台北信羲.桃概第一航廈等門市不提供兌换服務。'
        },{
          imgs: require("@/assets/imgs/product.png"),
          title: '大苑子·翡翠檸檬鞏固 濾心可用兌換券可用兌換券可用兌換券3',
          price: 200,
          details: '一年內不管要换幾次濾心樱花都幫你换I凡購買指定機種，還享獨家優惠',
          Precautions: ' 注意事項:<br />1.本公司保有最終更改決定權。<br />2.台北信羲.桃概第一航廈等門市不提供兌换服務。'
        },{
          imgs: require("@/assets/imgs/product.png"),
          title: '大苑子·翡翠檸檬鞏固 濾心可用兌換券4',
          price: 200,
          details: '一年內不管要换幾次濾心樱花都幫你换I凡購買指定機種，還享獨家優惠',
          Precautions: ' 注意事項:<br />1.本公司保有最終更改決定權。<br />2.台北信羲.桃概第一航廈等門市不提供兌换服務。'
        }
      ...
      latestEventsList: [
        {
          imgs: require("@/assets/imgs/product1.png"),
          title: '揪水友 趁新鮮',
  ...
  methods: {
    ...
    couponClose () {
      this.CouponNumber = ''
      this.CouponNumberStatus = false
      this.couponStatus = false
    },
    // 使用優惠卷確認彈窗關閉
    couponSubmit () {
      if (this.CouponNumber != '666666') {
        this.CouponNumberStatus = true
        return false
      }
      this.CouponNumber = ''
      this.CouponNumberStatus = false
    ...
    SearchClick () {
      if (this.searchValue) {
        ...
        if (this.active == 0) {
          this.ExchangeMallList = []
        } else if (this.active == 1) {
          this.latestEventsList = []
