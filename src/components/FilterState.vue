    ...
    <el-card class="card" :style="{ height: isHeight }">
      <div class="icon">
        ...
        <div class="title">
          ...
          <span>Filter No. {{ list.Sequence }}</span>
        </div>
      </div>
      <div
        class="filter-img"
        v-if="
          isRemainLife > 50 ||
            (isRemainLife === 50 && list.RemainDays >= 30) ||
            (isRemainLife < 50 && list.RemainDays < 30) ||
            isRemainLife < 50
        "
        :class="{
          'filter-img_2': (orange =
            isRemainLife === 50 && list.RemainDays >= 30),
          'filter-img_3':
            (isRemainLife < 50 && list.RemainDays < 30) || isRemainLife < 50
        }"
      >
      ...
      <a
        href="javascript:;"
        v-if="list.RemainDays >= 30"
        @click.stop="timeClick"
        >建議{{ list.RemainDays }}天後更換濾心</a
      >
      <a
        href="javascript:;"
        v-if="isRemainLife < 50 && list.RemainDays < 30"
        @click.stop="redClick"
        >點我開始更新</a
      >
...
export default {
  ...
  computed: {
    isRemainLife() {
      return +this.list.RemainLife.substring(
        0,
        this.list.RemainLife.length - 1
      );
    }
  },
  methods: {
